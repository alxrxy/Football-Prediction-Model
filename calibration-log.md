...
**Why the refit is not applied yet.** The correction is described elsewhere in this log as display-only, and it is
for the pick and the ranking, which both use the raw gap. But `blended` is built from the corrected probability and
`passes_stage1` is built from `blended`, so changing these offsets changes **which props carry a value flag**. On a
slate already in progress that is exactly the mid-slate swap the user asked to avoid. The `market_view` fix itself
is committed, because it only restores props that should always have been there and moves no existing line.
