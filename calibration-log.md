...
**H2, the market ahead of the model: not met, and the sign is wrong.** On the 30 graded leans, mean CLV is -0.2 pp (perm p 0.32). corr(|edge|, CLV) is +0.15: the line moved *toward* the big-edge leans (+2.8 pp at >= 4, n 15) and away from the small ones (-3.2 pp). Graded at the logged close instead of the stored line, the record is the same 9-20-1. The market didn't correct the model's big disagreements before kickoff. The leans lost on the field.
